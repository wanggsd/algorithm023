...
class Solution:
  def leastInterval(self, tasks, n):
    ...
    counts.sort()
    n_max = counts.pop()
    # idle time after assigning the most frequent task
    idle_time = (n_max - 1) * n
    while counts and idle_time > 0:
      # cap the decrement in case multiple tasks have the highest freq
      idle_time -= min(n_max - 1, counts.pop())
    # make sure idle_time = 0 when there are more than n types of taks
    idle_time = max(0, idle_time)
    # all tasks themselves takes 1 unit of time each
    return idle_time + len(tasks)


if __name__ == "__main__":
  print(Solution.leastInterval(None, ["A","A","A","B","B","B"], 2))
  print(Solution.leastInterval(None, ["A","A","A","B","B","B"], 0))
  print(Solution.leastInterval(None, ["A","A","A","A","A","A","B","C","D","E","F","G"], 2))
  ...
  print(Solution.leastInterval(None, tasks, 1))
